...
export interface IMoveFromHereToThere {
    MoveFileFromThisContainerTo: (targetBucket: string, targetKey: string) => void;
}

export class UnCategorizedImagesStore extends S3Core implements IMoveFromHereToThere {
    constructor() {
    ...
    public async SaveDalleUrlsToS3(urls: string[], metaData: ImageBatchMetaData, tags: S3.Tag[] = []) {
        ...
        for (const url of urls) {
            ...
            const imageLocationDetails = await this.SaveDalleImageFileToS3(imageTransfer);
        ...
        await this.SaveSerializedDataToS3(serializedMetaData, metaFileName, metaFilePrefix);
        return data;
    }

    private async SaveSerializedDataToS3(serializedData: string, fileName: string, prefix: string) {
        const buffer = Buffer.from(serializedData, 'utf-8');
        await this.writeFiles([{ name: fileName, data: buffer, contentType: 'text/*' }], this.uploadOptions, prefix);
    }
    ...
    public async MoveFileFromThisContainerTo(targetBucket: string, targetKey: string) {
